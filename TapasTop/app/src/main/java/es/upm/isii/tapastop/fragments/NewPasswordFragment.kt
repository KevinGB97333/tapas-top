		...
		binding.apply {
			...
			newPasswordText.addTextChangedListener {
				if (it.toString().isBlank()) {
					...
					if (newPasswordConfirmText.text.toString()
							.isNotBlank() && it.toString() != newPasswordConfirmText.text.toString()
					) {
			...
			newPasswordConfirmText.addTextChangedListener {
				if (it.toString().isBlank() || (newPasswordText.text.toString()
						.isNotBlank() && it.toString() != newPasswordText.text.toString())
				) {
					newPasswordConfirm.error = getString(R.string.passwords_unmatch_msg)
